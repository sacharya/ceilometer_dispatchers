    ...
    def record_events(self, events):
        ...
        for event in events:
            try:
                ...
                LOG.debug('Event Message: %s', event)
                        ...
                        if(trait[0] == "service"):
                            try:
                                ...
                                if '.' in service:
                                    ...
                                    if service_prefix in ["compute", "network", "volume", "objectstorage", "image", "identity"]:
                                        service = service_prefix
                                    else:
                                        if "nova" in service.split('.')[1]:
                                            service = "compute"
                            except:
                                pass
                        service_name = event_type.split(".")[0]
                        if service_name != service and service_name in ["compute", "network", "volume", "objectstorage", "image", "identity"]:
                            service = service_name
                    except:
                        pass
                timeis = event["generated"]
                import time
                ts = time.strptime(timeis, '%Y-%m-%dT%H:%M:%S.%f')
                timestamp = int(time.mktime(ts))*1000000000

                value = 1
                #event_json = 'events,event_name=%s,host=%s value=%s %s' %(event_type, value, timestamp )
                if tenant_id is not None:
                    event_json = 'events,event_name=%s,service=%s,tenant_id=%s,user_id=%s value=%s %s' %(event_type, service, tenant_id, user_id, value, timestamp )
                else:
                    event_json = 'events,event_name=%s,service=%s  value=%s %s' %(event_type, service, value, timestamp )
                headers = {}
                LOG.debug('Influx Message: %s', event_json)
